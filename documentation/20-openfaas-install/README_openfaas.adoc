...
7) Continue with the previous set:
    ...
    [source,bash]
    ----
    echo $(microk8s kubectl -n openfaas get secret basic-auth -o jsonpath="{.data.basic-auth-password}" | base64 --decode)
    ----
...
== Install OpenFaaS in microk8s and how to access the UI
  ...
  Include this variable in ~/.bashrc:
  [source,bash]
  ----
  ...
  [source,bash]
  ----
  ssh -L localhost:8080:localhost:31112 paula@130.149.223.203
  curl http://127.0.0.1:31112
  ----
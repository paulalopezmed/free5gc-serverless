...
import (
	...
	"handler/function/static/ausf/ausfin/sbi/producer"
...
func HTTPEapAuthMethod(ctx *gin.Context) {
	...
	if err != nil {
		...
		ausflogger.Auth5gAkaLog.Errorf("Get Request Body error: %+v", err)
	...
	if err != nil {
		...
		ausflogger.Auth5gAkaLog.Errorln(problemDetail)
	...
	if err != nil {
		ausflogger.Auth5gAkaLog.Errorln(err)
...
func HTTPUeAuthenticationsPost(ctx *gin.Context) {
	fmt.Println("ue-authentications request")
	...
	if err != nil {
		...
		ausflogger.UeAuthLog.Errorf("Get Request Body error: %+v", err)
		ctx.JSON(http.StatusInternalServerError, problemDetail)
		return
	}
	...
	if err != nil {
		...
		ausflogger.UeAuthLog.Errorln(problemDetail)
		ctx.JSON(http.StatusBadRequest, rsp)
		return
	}
	...
	responseBody, err := openapi.Serialize(rsp.Body, "application/json")
	if err != nil {
		ausflogger.UeAuthLog.Errorln(err)
		problemDetails := models.ProblemDetails{
			Status: http.StatusInternalServerError,
			Cause:  "SYSTEM_FAILURE",
			Detail: err.Error(),

...
func SendRegisterNFInstance(nrfUri, nfInstanceId string, profile models.NfProfile) (string, string, error) {
	...
	for {
		if _, resTmp, err := client.NFInstanceIDDocumentApi.RegisterNFInstance(context.TODO(), nfInstanceId,
			...
			ausflogger.ConsumerLog.Errorf("AUSF register to NRF Error[%v]", err)
		...
		defer func() {
			if resCloseErr := res.Body.Close(); resCloseErr != nil {
				ausflogger.ConsumerLog.Errorf("AUSF NFInstanceIDDocumentApi response body cannot close: %+v", resCloseErr)
			}
		}()
...
func SendDeregisterNFInstance() (*models.ProblemDetails, error) {
	ausflogger.ConsumerLog.Infof("Send Deregister NFInstance")
	...
	if err == nil {
		...
		defer func() {
			if resCloseErr := res.Body.Close(); resCloseErr != nil {
				ausflogger.ConsumerLog.Errorf("NFInstanceIDDocumentApi response body cannot close: %+v", resCloseErr)
			}
		}()

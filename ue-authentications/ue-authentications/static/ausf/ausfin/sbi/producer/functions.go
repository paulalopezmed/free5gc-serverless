...
func KDF5gAka(param ...string) hash.Hash {
	...
	if p0len, err := strconv.Atoi(param[2]); err != nil {
		ausflogger.AuthELog.Warnf("atoi failed: %+v", err)
	} else {
...
func CalculateAtMAC(key []byte, input []byte) []byte {
	...
	if _, err := h.Write(input); err != nil {
		ausflogger.AuthELog.Errorln(err.Error())
	}
	sum := h.Sum(nil)
	return sum[:16]
}
...
func EapEncodeAttribute(attributeType string, data string) (string, error) {
	...
	default:
		ausflogger.AuthELog.Errorf("UNKNOWN attributeType %s\n", attributeType)
		return "", nil
	}
...
func eapAkaPrimePrf(ikPrime string, ckPrime string, identity string) ([]byte, []byte, []byte, []byte, []byte) {
	...
	if keyTmp, err := hex.DecodeString(keyAp); err != nil {
		ausflogger.AuthELog.Warnf("Decode key AP failed: %+v", err)
	} else {
		key = keyTmp
	}
...
func decodeEapAkaPrime(eapPkt []byte) (*ausf_context.EapAkaPrimePkt, error) {
	...
	for i := 3; i < dataLen; i += attrLen {
		...
		case ausf_context.AT_RES_ATTRIBUTE:
			ausflogger.AuthELog.Tracef("Decoding AT_RES\n")
		...
		case ausf_context.AT_MAC_ATTRIBUTE:
			ausflogger.AuthELog.Tracef("Decoding AT_MAC\n")
			if attrLen != 20 {
		...
		case ausf_context.AT_KDF_ATTRIBUTE:
			ausflogger.AuthELog.Tracef("Decoding AT_KDF\n")
			if attrLen != 4 {
		...
		case ausf_context.AT_AUTS_ATTRIBUTE:
			ausflogger.AuthELog.Tracef("Decoding AT_AUTS\n")
			if attrLen != 16 {
		...
		case ausf_context.AT_CLIENT_ERROR_CODE_ATTRIBUTE:
			ausflogger.AuthELog.Tracef("Decoding AT_CLIENT_ERROR_CODE\n")
			if attrLen != 4 {
			...
			attributes[attrType] = decodeAttr
		default:
			ausflogger.AuthELog.Tracef("attribute type %x skipped\n", attrType)
		}
	}

	switch decodePkt.Subtype {
	case ausf_context.AKA_CHALLENGE_SUBTYPE:
		ausflogger.AuthELog.Tracef("Subtype AKA-Challenge\n")
	...
	case ausf_context.AKA_AUTHENTICATION_REJECT_SUBTYPE:
		ausflogger.AuthELog.Tracef("Subtype AKA-Authentication-Reject\n")
		if len(attributes) != 0 {
			return nil, fmt.Errorf("AKA-Authentication-Reject attributes error")
		}
	case ausf_context.AKA_SYNCHRONIZATION_FAILURE_SUBTYPE:
		ausflogger.AuthELog.Tracef("Subtype AKA-Synchronization-Failure\n")
		if len(attributes) != 2 {
	...
	case ausf_context.AKA_NOTIFICATION_SUBTYPE:
		ausflogger.AuthELog.Tracef("Subtype AKA-Notification\n")
	case ausf_context.AKA_CLIENT_ERROR_SUBTYPE:
		ausflogger.AuthELog.Tracef("Subtype AKA-Client-Error\n")
		if len(attributes) != 1 {
	...
	default:
		ausflogger.AuthELog.Tracef("subtype %x skipped\n", decodePkt.Subtype)
	}
...
func ConstructFailEapAkaNotification(oldPktId uint8) string {
	...
	if attrHexTmp, err := hex.DecodeString(attribute); err != nil {
		ausflogger.AuthELog.Warnf("Decode attribute failed: %+v", err)
	} else {
		attrHex = attrHexTmp
	}
...
func getUdmUrl(nrfUri string) string {
	...
	if err != nil {
		ausflogger.UeAuthLog.Errorln("[Search UDM UEAU] ", err.Error())
		...
		if len(udmInstance.Ipv4Addresses) > 0 && udmInstance.NfServices != nil {
			...
			udmUrl = string(ueauService.Scheme) + "://" + ueauEndPoint.Ipv4Address + ".free5gc.svc.cluster.local:" + strconv.Itoa(int(ueauEndPoint.Port))
		}
	} else {
		ausflogger.UeAuthLog.Errorln("[Search UDM UEAU] len(NfInstances) = 0")
	}
	return udmUrl
}
...
func sendAuthResultToUDM(id string, authType models.AuthType, success bool, servingNetworkName, udmUrl string) error {
	...
	defer func() {
		if rspCloseErr := rsp.Body.Close(); rspCloseErr != nil {
			ausflogger.ConsumerLog.Errorf("ConfirmAuth Response cannot close: %v", rspCloseErr)
		}
	}()
...
func logConfirmFailureAndInformUDM(id string, authType models.AuthType, servingNetworkName, errStr, udmUrl string) {
	if authType == models.AuthType__5_G_AKA {
		ausflogger.Auth5gAkaLog.Infoln(errStr)
		...
		ausflogger.AuthELog.Infoln(errStr)
		if sendErr := sendAuthResultToUDM(id, authType, false, "", udmUrl); sendErr != nil {

...
const (
	...
	//Configuration file path must be changed
	//AusfDefaultConfigPath = "./config/ausfcfg.yaml"
	AusfDefaultConfigPath         = "/var/openfaas/secrets/ausfcfg.yml"
	AusfSbiDefaultIPv4            = "127.0.0.9"
	AusfSbiDefaultPort            = 8080
...
func (c *Configuration) validate() (bool, error) {
	if sbi := c.Sbi; sbi != nil {
		if result, err := sbi.validate(); err != nil {
			fmt.Println("config validate starting")
...
func (c *Config) SetLogEnable(enable bool) {
	...
	if c.Logger == nil {
		ausflogger.CfgLog.Warnf("Logger should not be nil")
		c.Logger = &Logger{
			Enable: enable,
			Level:  "info",
		}
